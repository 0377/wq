...
class Goods_EweiShopV2Page extends AppMobilePage
{
    ...
    public function openGroups()
    {
        ...
        $goods['fightnum'] = pdo_fetchcolumn("select count(1) from ".tablename('ewei_shop_groups_order')." where goodid = :goodid and uniacid = :uniacid and ladder_id=:ladder_id and deleted = 0 and is_team = 1 and status > 0 ",
            array(":goodid"=>$goods['id'],':uniacid'=>$_W['uniacid'],':ladder_id' => $goods['ladder_id']));
        $goods['fightnum'] = $goods['teamnum'] + $goods['fightnum'];
        $goods = set_medias($goods, 'thumb');
