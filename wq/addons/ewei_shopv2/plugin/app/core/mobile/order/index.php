...
//            print_r($row['goods']);exit;
    ...
    function express()
    {
        ...
        if(!empty($cycelid)){
            ...
            $order = pdo_fetch("select expresscom,expresssn,addressid,status,express from " . tablename('ewei_shop_order') . ' where id=:id and uniacid=:uniacid and openid=:openid limit 1'
                , array(':id' => $orderid, ':uniacid' => $uniacid, ':openid' => $openid));
            if (empty($order)) {
                app_error(AppError::$OrderNotFound);
            }
