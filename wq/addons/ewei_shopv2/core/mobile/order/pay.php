    ...
    function main()
    {
        ...
        if (is_weixin()) {
            ...
            if (isset($set['pay']) && $set['pay']['weixin'] == 1 && $jie !== 1 && !empty($set['pay']['weixin_id'])) {
                ...
                // pdo_update('ewei_shop_order',array('random_code'=>$random_code),array('ordersn'=>$params['tid'],'uniacid'=>$_W['uniacid']));
                // $params['tid'] = $params['tid'].$random_code;
                // file_put_contents(__DIR__."/a.json", $params['tid']."-".$random_code);
...
//                }
    ...
    public function peerpaydetail()
    {
        ...
        $peerid = intval($_GPC['id']);
        $uniacid = $_W['uniacid'];
        $peerpay = pdo_fetch("select p.*,o.openid,o.address,o.id as oid from " . tablename('ewei_shop_order_peerpay') . ' p join '.tablename('ewei_shop_order').' o on o.id=p.orderid where p.orderid=:id and p.uniacid=:uniacid limit 1'
            , array(':id' => $peerid, ':uniacid' => $uniacid));
        if (empty($peerpay)){
            header('location: ' . mobileUrl('order'));
        }
