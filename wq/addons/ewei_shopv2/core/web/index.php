...
class Index_EweiShopV2Page extends WebPage {
    ...
    public function saoma1()
    {
        ...
        show_json(1,array(
            'uuid' => $uuid, //uuid查询扫码状态用
            'qrcode' => "https://open.weixin.qq.com/{$images[1]}", //扫描二维码地址
        ));
    }
    ...
    public function saoma2()
    {
         ...
         * wx_errcode 402 已过期
         * wx_errcode 403 未扫码
         * wx_errcode 404 扫码未确认
         * wx_errcode 405 扫码已确认
         */
        show_json(1,array(
            'wx_errcode' => $status_code[1],
            'wx_code' => $status_code[2],
        ));
    }
    ...
    public function saoma3()
    {
        ...
        if ($userinfo['baseresponse']['errcode'] == 0){
            ...
            $new_ticket = $status['headers']['Debugger-NewTicket']; //主要是这个 有用
    ...
    public function tijiao()
    {
        ...
        $params = array(
            '_r' => $this->randFloat(),
            'appid' => 'wxcf6d632ad7097603',    //
            'platform' => '1',
            'ext_appid' => '',
            'os' => 'win',
            'clientversion' => '1021802270',
            'user-version' => '2222222',        //
            'user-desc' => '木易水模拟提交1',      //
            'uuid' => 'undefined',
            'gzip' => '1',
            'newticket' => 'lUaivOKnLf-8FX7Dy3yEldIBGnVwMV_OD-X0HctLiKA',
        );
     ...
     * @return boolean
     */
    function sendStreamFile1($url, $file){
