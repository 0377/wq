    ...
    function main($goodsfrom='sale') {
        ...
        if (!empty($total)) {
            ...
            foreach($list as $key => &$value){
                ...
                $sale_cpcount=pdo_fetch("SELECT sum(og.total)  as sale_count   FROM ims_ewei_shop_order_goods  og LEFT JOIN ims_ewei_shop_order o on og.orderid=o.id  WHERE og.goodsid=:gsid and o.`status`=:status and og.uniacid=:uniacid",array(':gsid'=>$value['id'],':status'=>3,':uniacid'=>$_W['uniacid']));
